...
def calculate_scheduled_outage_factor(curtailments,resources,nminutes=5):
    '''
    Calculates the monthly scheduled outage factors for each resource in the
    input curtailment dataframe as:
        USOF = (SOH + ESDH) / PH
    ...
    print('df0 columns: {}'.format(', '.join(df0.columns)))
    print('df1 columns: {}'.format(', '.join(df1.columns)))
    ...
    df3.loc[:,['RESOURCE ID','OUTAGE MRID','MONTH','OUTAGE TYPE','NATURE OF WORK','CURTAILMENT MW','CURTAILMENT DURATION']].to_csv(r'M:\Users\RH2\src\caiso_curtailments\storage_sof\CurtailmentDurationsByMRID.csv',index=False)
    ...
    filtered_indices = df1.loc[:,'OUTAGE TYPE'] == 'NONE'
    for p in maintenance_codes:
        filtered_indices = filtered_indices | ((df1.loc[:,'OUTAGE TYPE']==p[0])&(df1.loc[:,'NATURE OF WORK']==p[1]))
    df1 = df1.loc[filtered_indices,:]
    ...
    # Calculate equivalent scheduled derated hours (ESDH) for each time block,
    # applying the formula ESDH = hrs * MW / NMC:
    df1.loc[:,'ESDH'] = nminutes / 60 * df1.loc[:,'CURTAILMENT MW'] / df1.loc[:,'NET DEPENDABLE CAPACITY']
    ...
    df1.loc[:,'ESDH'] = df1.loc[:,'ESDH'].fillna(value=0)

    # Calculate monthly ESOF within each time block, applying the formula
    # ESOF = (SOH+ESDH)/(total hours in month), with SOH accounted for in
    # ESOF where curtailment MW=NMC:
    df1.loc[:,'ESOF'] = df1.loc[:,'ESDH'] / df1.loc[:,'MONTH DURATION']
    ...
    df3.loc[:,'ESOF'] = df3.loc[:,'ESOF'].fillna(value=0)
...
if __name__=='__main__':
    ...
    storage_curtailments_path = Path('M:\\Users\\RH2\\src\\caiso_curtailments\\storage_ucap\\curtailed_storage_resources.csv')
    if not storage_curtailments_path.is_file():
        ...
        # df0.loc[:,['OUTAGE TYPE','NATURE OF WORK']].groupby(['OUTAGE TYPE','NATURE OF WORK']).first().reset_index().to_csv(r'M:\Users\RH2\src\caiso_curtailments\storage_ucap\OutageCodes.csv',index=False)
        ...
        df1 = pd.read_csv(r'M:\Users\RH2\src\caiso_curtailments\storage_ucap\MasterCapabilityList_2024-01-22.csv')
        df1 = df1.rename(columns={'RESOURCE_ID':'RESOURCE ID','ENERGY_SOURCE':'ENERGY SOURCE'})
        df0 = df0.merge(df1,how='left',on='RESOURCE ID')
        df0.loc[(df0.loc[:,'ENERGY SOURCE']=='LESR'),[
            'OUTAGE MRID',
            'RESOURCE ID',
            'ENERGY SOURCE',
            'OUTAGE TYPE',
    ...
    df2 = pd.read_csv(r'M:\Users\RH2\src\caiso_curtailments\storage_ucap\MasterCapabilityList_2024-01-22.csv')
    for nminutes in [1]:
        df3 = calculate_scheduled_outage_factor(df0,df2,nminutes)
        df3.to_csv(r'M:\Users\RH2\src\caiso_curtailments\storage_ucap\MonthlySOF_{}min.csv'.format(nminutes),index=False)
